                    ...
                    {(() => {
                      ...
                      React.useEffect(() => {
                        const fetchAllPayments = async () => {
                          ...
                          try {
                            console.log('🐛 DEBUG: Fetching ALL payment records for student');
                            console.log('🐛 Student ID:', scannedProfile.id);
                            console.log('🐛 Student Type:', scannedProfile.type);
                            console.log('🐛 Current Year:', currentYear);
                            
                            // Determine correct userId for query
                            let debugUserId: number;
                            if (scannedProfile.type === "student") {
                              debugUserId = scannedProfile.id;
                            } else if (scannedProfile.type === "child") {
                              debugUserId = scannedProfile.parentId || scannedProfile.id;
                            } else {
                              debugUserId = scannedProfile.id;
                            }
                            
                            console.log('🐛 Determined userId for query:', debugUserId);
                            
                            // Fetch payment records for all months of current year
                            const debugPromises = [];
                            for (let month = 1; month <= 12; month++) {
                              const promise = fetch(`/api/students/${debugUserId}/payment-status/${currentYear}/${month}`, {
                                credentials: 'include'
                              })
                              ...
                              debugPromises.push(promise);
                            }
                            
                            const allPayments = await Promise.all(debugPromises);
                            
                            console.log('🐛 ===== ALL STUDENT PAYMENT RECORDS DEBUG =====');
                            console.log('🐛 Student:', scannedProfile.name);
                            console.log('🐛 Student ID:', scannedProfile.id);
                            console.log('🐛 Student Type:', scannedProfile.type);
                            console.log('🐛 Query User ID:', debugUserId);
                            console.log('🐛 Current Year:', currentYear);
                            
                            allPayments.forEach(({ month, data }) => {
                              if (data && data.id) {
                                console.log(`🐛 Month ${month}: ✅ RECORD EXISTS`);
                                console.log(`   - Database ID: ${data.id}`);
                                console.log(`   - User ID: ${data.userId}`);
                                console.log(`   - Student Type: ${data.studentType}`);
                                console.log(`   - Amount: ${data.amount || 'NULL'}`);
                                console.log(`   - Is Paid: ${data.isPaid ? 'YES' : 'NO'}`);
                                console.log(`   - Paid Date: ${data.paidAt || 'NULL'}`);
                                console.log(`   - Notes: ${data.notes || 'NULL'}`);
                              } else {
                                console.log(`🐛 Month ${month}: ❌ NO RECORD`);
                              }
                            });
                            
                            const paidMonths = allPayments
                              .filter(({ data }) => data && data.isPaid)
                              .map(({ month }) => month);
                              
                            const unpaidMonths = allPayments
                              .filter(({ data }) => data && !data.isPaid)
                              .map(({ month }) => month);
                              
                            const noRecordMonths = allPayments
                              .filter(({ data }) => !data)
                              .map(({ month }) => month);
                            
                            console.log(`🐛 SUMMARY:`);
                            console.log(`   - Paid months (${paidMonths.length}):`, paidMonths);
                            console.log(`   - Unpaid months (${unpaidMonths.length}):`, unpaidMonths);
                            console.log(`   - No record months (${noRecordMonths.length}):`, noRecordMonths);
                            console.log('🐛 ===============================================');
                            
                          } catch (error) {
                            console.error('🐛 DEBUG ERROR: Failed to fetch payment records:', error);
                          }
                        };
                        
                        fetchAllPayments();
                      }, [scannedProfile?.id, currentYear]);
