...
export default function Groups() {
  ...
  const handleTogglePayment = (studentId: number) => {
    ...
    if (currentPayment?.isVirtual && !currentPayment?.mustPay) {
      toast({ title: 'لا يوجد دفع مطلوب لهذا الشهر حتى الآن', variant: 'destructive' });
      return;
    }
              ...
              {existingGroupsFilter && (() => {
                ...
                if (existingGroupsFilter === 'custom') {
                  ...
                  filteredGroups = adminCreatedGroups.filter(group => {
                    ...
                    console.log(`DEBUG: Checking group "${group.name}" (ID: ${group.id}) with subjectId ${group.subjectId}:`, {
                      foundModule: teachingModule ? true : false,
                      hasSchoolId: teachingModule?.schoolId ? true : false,
                    ...
                    if (!teachingModule || !teachingModule.schoolId) {
                      console.log(`DEBUG: Group "${group.name}" EXCLUDED: No teaching module or no schoolId`);
                      return false;
                    }
                    ...
                    console.log(`DEBUG: Group "${group.name}" subject name: "${subjectName}", isStandardSubject: ${isStandardSubject}`);
                    
                    // Only show truly custom subjects (non-standard subjects with schoolId)
                    const shouldInclude = !isStandardSubject;
                    console.log(`DEBUG: Group "${group.name}" ${shouldInclude ? 'INCLUDED' : 'EXCLUDED'} in custom groups`);
                    
                    return shouldInclude;
                  });
                  
                  console.log('DEBUG: Final filtered custom groups:', filteredGroups.length, filteredGroups.map(g => ({ id: g.id, name: g.name })));
                } else {
                      ...
                      <div className="space-y-2">
                        {availableStudents
                          ...
                          .filter(student => {
                            ...
                            const groupGrade = selectedAdminGroup.grade; // From teaching module
                            
                            // Function to map specializations to year levels (Algerian system)
                            const getYearFromSpecialization = (specialization: string): string[] => {
                              const spec = specialization?.toLowerCase();
                              
                              // Secondary specializations (3rd year)
                              if (spec?.includes('تسيير واقتصاد') || spec?.includes('تسيير') || spec?.includes('اقتصاد')) {
                                return ['الثالثة ثانوي', 'السنة الثالثة ثانوي'];
                              }
                              if (spec?.includes('علوم تجريبية') || spec?.includes('تجريبية')) {
                                return ['الثالثة ثانوي', 'السنة الثالثة ثانوي'];
                              }
                              if (spec?.includes('رياضيات') || spec?.includes('تقني رياضي')) {
                                return ['الثالثة ثانوي', 'السنة الثالثة ثانوي'];
                              }
                              if (spec?.includes('آداب وفلسفة') || spec?.includes('آداب') || spec?.includes('فلسفة')) {
                                return ['الثالثة ثانوي', 'السنة الثالثة ثانوي'];
                              }
                              if (spec?.includes('لغات أجنبية') || spec?.includes('لغات')) {
                                return ['الثالثة ثانوي', 'السنة الثالثة ثانوي'];
                              }
                              
                              // If no specialization match, return empty array
                              return [];
                            };
                            
                            // Check explicit year mentions in group text first
                            ...
                            if (groupGrade && groupLevel === 'الثانوي') {
                              ...
                              if (requiredYears.length > 0) {
                                return requiredYears.some(year => studentGrade.includes(year));
                              }
                            }
