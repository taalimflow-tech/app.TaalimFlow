...
export class DatabaseStorage implements IStorage {
  ...
  async getStudentGroups(userId: number, schoolId: number): Promise<any[]> {
    try {
      // Get groups the student is assigned to (both old system and new mixed system)
      const studentGroups = await db
        ...
        .leftJoin(teachingModules, eq(groups.subjectId, teachingModules.id))
        .leftJoin(users, eq(groups.teacherId, users.id))
        .leftJoin(groupUserAssignments, eq(groupUserAssignments.groupId, groups.id))
        .leftJoin(groupMixedAssignments, eq(groupMixedAssignments.groupId, groups.id))
        .where(
          and(
            eq(groups.schoolId, schoolId),
            or(
              eq(groupUserAssignments.userId, userId),
              eq(groupMixedAssignments.studentId, userId)
            )
          )
        );

      // For each group, get attendance data and scheduled dates
      const enrichedGroups = await Promise.all(
        studentGroups.map(async (group) => {
          ...
          // Get scheduled dates for this group
          const scheduledDates = await this.getGroupScheduledLessonDates(group.groupId);
